...
saveRDS(object=tweets,file="vignettes/data/tweets.rds")
```
```{r,echo=FALSE}
...
tweets <- tweets %>% mutate( is_retweet_of_us = (retweet_status_id == 803931871662972928))
...
ts3 <- rbind(ts2 %>% rename(n=n_retweets) %>% select(-n_noretweets) %>% mutate(group="retweet"),
             ts2 %>% rename(n=n_noretweets) %>% select(-n_retweets) %>% mutate(group="no_retweet"))
...
```{r}
p + geom_rect(data = filter(conference_program_slots, to < "2016-11-30 17:30:00 CET"),
...
baseline <- 7
tweet_sts <- surveillance::sts(observed = ts2$n_retweets + ts2$n_noretweets, # weekly number of cases
